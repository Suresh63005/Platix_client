...
const CreateOrganization = () => {
  ...
  const [newService, setNewService] = useState({ id: "", name: "", price: "" });
  ...
  // Fetch organization data, services, and org types with cleanup
  useEffect(() => {
    let isMounted = true;
        ...
        if (orgResponse?.data?.data) {
          ...
          if (orgData.services) {
            const servicesWithNames = orgData.services.map((service) => ({
              id: service?.serviceDetail?.id || service?.id,
  ...
  // Cleanup for file preview URLs
  useEffect(() => {
    return () => {
      if (formData.imgPreview) {
  ...
  const dentistId = useMemo(
    () => orgType.find((option) => option.label.toLowerCase() === "dentist")?.value,
    [orgType]
  );
  ...
  const handleImageUpload = (e) => {
    ...
    const maxSize = 1 * 1024 * 1024;
    const oversizedImages = filess.filter((file) => file.size > maxSize);
    ...
    if (filess.length + imageExtra.length > 3) {
  ...
  const onSubmit = async (data) => {
    setloading(true);
    try {
      console.log("Form submission started", { userServices, addresses, vendorData });
      ...
      if (!hasAddressProof && watch("organizationType_id") !== dentistId) {
      ...
      form.append("fileextras", JSON.stringify(imageExtra));
  ...
  async function handleOrginazationtypeid(id) {
    try {
      ...
      console.error("Error in handleOrginazationtypeid:", error);
    }
  }
  ...
  }, [aadhaar, drivingLicense, voterId, passportNumber, setError, clearErrors, watch]);
            ...
            <div className="flex-1 overflow-y-auto p-3">
              <div className="bg-white border border-[#EAEAFF] shadow-md rounded-lg p-6 mb-6">
                ...
                <form className="grid gap-4" onSubmit={handleSubmit(onSubmit)}>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                        ...
                        <div className="flex gap-2">
                          ...
                          <InputField
                            ...
                            {...register("googleCoordinates.longitude", {
                              ...
                              pattern: {
                                value: REGEX_PATTERNS.longitude,
                    ...
                    <div>
                      ...
                      {errors.googlemaplink && (
                        <p className="text-red-500 text-xs">{errors.googlemaplink.message}</p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      />
                      {errors.whatsapp && (
                        <p className="text-red-500 text-xs">{errors.whatsapp.message}</p>
                      )}
                    </div>
                    ...
                    {watch("organizationType_id") === dentistId && (
                      <div>
                        ...
                        {errors.registrationId && (
                          <p className="text-red-500 text-xs">{errors.registrationId.message}</p>
                        )}
                      </div>
                    )}
                  </div>
                    ...
                    <div>
                      <InputField
                        ...
                        {...register("accountNumber", {
                          ...
                          pattern: {
                            value: REGEX_PATTERNS.accountNumber,
                    ...
                    <div>
                      <InputField
                        ...
                        {...register("accountHolder", {
                          ...
                          pattern: {
                            value: REGEX_PATTERNS.accountHolder,
                      ...
                      {errors.accountHolder && (
                        <p className="text-red-500 text-xs">{errors.accountHolder.message}</p>
                      )}
                    </div>
                    <div>
                      <InputField
                      ...
                      />
                      {errors.ifscCode && (
                        <p className="text-red-500 text-xs">{errors.ifscCode.message}</p>
                      )}
                    </div>
                    <div>
                      <InputField
                  ...
                  {watch("organizationType_id") !== dentistId && (
                    <div>
                      <h3 className="text-lg font-bold mb-4 mt-6">Business Details</h3>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                        <div>
                        ...
                        <div>
                          ...
                          {errors.vendorData?.pan && (
                            <p className="text-red-500 text-xs">{errors.vendorData.pan.message}</p>
                          )}
                        </div>
                          ...
                          <>
                            <div>
                              ...
                              {errors.vendorData?.gst && (
                                <p className="text-red-500 text-xs">{errors.vendorData.gst.message}</p>
                              )}
                            </div>
                            <div>
                              <InputField
                      ...
                      {errors.addressProof && (
                        <p className="text-red-500 text-xs mt-2">{errors.addressProof.message}</p>
                      )}
                    </div>
                  )}
                      ...
                      <div className="flex flex-col sm:flex-row gap-4 items-center mb-6">
                        <div className="w-full sm:w-1/2">
                          <Select
                            ...
                            value={
                              newService.id
                                ? { id: newService.id, value: newService.id, label: newService.name }
                                : null
                            }
                            onChange={(selectedOption) =>
                              setNewService({
                        ...
                        <div className="flex gap-2">
                          ...
                          <span
                            onClick={() => setNewService({ id: "", name: "", price: "" })}
                            className={`px-7 py-1 border rounded-md ${
                              mode1 !== "view"
                                ? "bg-white text-gray-700 border-gray-700 cursor-pointer"
                                : "bg-gray-200 text-gray-400 border-gray-300 cursor-not-allowed pointer-events-none"
