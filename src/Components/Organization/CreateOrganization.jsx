...
const CreateOrganization = () => {
  ...
  const dentistId = orgType.find((option) => option.label.toLowerCase() === "dentist")?.value;
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Validate address proof only if organization type is not Dentist
      if (data.organizationType_id !== dentistId) {
      ...
      // Validate registrationId for Dentist
      if (data.organizationType_id === dentistId && !data.registrationId) {
        setError("registrationId", {
          type: "manual",
          message: "Registration ID is required for Dentist.",
        });
        setloading(false);
        return;
      }
  ...
  const organizationTypeId = watch("organizationType_id");

  // Validate address proof fields only for non-Dentist organization types
  useEffect(() => {
    if (organizationTypeId !== dentistId) {
      ...
      clearErrors("addressProof"); // Clear address proof errors for Dentist
    }
  }, [aadhaar, drivingLicense, voterId, passportNumber, organizationTypeId, setError, clearErrors, dentistId]);
